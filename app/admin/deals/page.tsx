...
function ProspectDetail({ prospect, token, keepaOn, onUpdate, onDelete }: {
  prospect: Prospect; token: string; keepaOn: boolean;
  ...
  const field = (label: string, key: keyof Prospect, type = "text") => (
    <label style={{ display: "block", fontSize: 12, marginBottom: 8 }}>
      <span style={{ color: muted, fontWeight: 600 }}>{label}</span>
      <input
        type={type} defaultValue={(prospect[key] as string) ?? ""}
        onBlur={(e) => { if (e.target.value !== prospect[key]) patch({ [key]: e.target.value }); }}
...
export default function DealDeskPage() {
  ...
  const createProspect = useCallback(async () => {
    ...
    if (r.ok) {
      ...
      setProspects((ps) => [{ ...p, products: p.products ?? [] }, ...ps]);
      setSelectedId(p.id);
    ...
    >
      <div style={{ maxWidth: 1240, margin: "0 auto", padding: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: selected ? "minmax(0,420px) 1fr" : "1fr", gap: 20, alignItems: "start" }}>
          ...
          {selected && (
            <ProspectDetail
              prospect={selected} token={token} keepaOn={keepaOn}
